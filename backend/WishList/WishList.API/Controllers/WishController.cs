...
namespace WishList.API.Controllers
{
    ...
    public class WishController : ControllerBase
    {
        ...
        {
            var wish = new WishEntity(request.Name, request.Price, request.Description);
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] GetWishRequest request, CancellationToken clt)
        {
            var wishQuery =  _dbContext.Wish
                .Where(n => !string.IsNullOrEmpty(request.Search) &&
                n.Name.ToLower().Contains(request.Search.ToLower()) || n.Description.ToLower().Contains(request.Search.ToLower())); //15.51 закончит тайм код)))
            ...
            var wishDtos = await wishQuery
                .Select(n => new WishDTO(n.Id, n.Name, n.Price, n.Description, n.Created))
                .ToListAsync(clt);
